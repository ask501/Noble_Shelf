...
# ── バックアップ復元ダイアログ（app.py）───────────────────────
BACKUP_REASON_LABELS: dict[str, str] = {
    "manual": "手動",
    "libchange_before": "ライブラリ変更（前）",
    "libchange_after": "ライブラリ変更（後）",
    "pre_restore": "復元前",
}
BACKUP_DIALOG_BG = COLOR_BG_BASE
BACKUP_DIALOG_FG = COLOR_TEXT_MAIN
BACKUP_DIALOG_ITEM_HOVER = COLOR_HOVER
BACKUP_DIALOG_ITEM_SELECT = COLOR_ACCENT
...
# サムネストリップ：現在ページのハイライト背景（アクセントの塗り・視認性重視）
VIEWER_THUMB_STRIP_SELECTED_BG = "rgba(154, 127, 255, 0.5)"
# サムネストリップ：現在ページ枠（COLOR_ACCENT より明るい紫）
VIEWER_THUMB_STRIP_ACCENT_BRIGHT = "#C8B8FF"
VIEWER_THUMB_STRIP_CURRENT_PAGE_BORDER = VIEWER_THUMB_STRIP_ACCENT_BRIGHT
# 全画面サムネイルオーバーレイ（viewer.py）
VIEWER_OVERLAY_HIGHLIGHT_BORDER = COLOR_ACCENT
VIEWER_OVERLAY_HIGHLIGHT_BG = COLOR_HOVER
VIEWER_OVERLAY_PLACEHOLDER_BG = COLOR_CARD_PLACEHOLDER
