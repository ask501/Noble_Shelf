...
# db.get_all_books() / get_all_books_order_by_added_desc / search_books（同形式）のタプル列インデックス（0始まり）
BOOK_ROW_NAME = 0
BOOK_ROW_CIRCLE = 1
BOOK_ROW_TITLE = 2
BOOK_ROW_PATH = 3
...
BOOK_ROW_IS_DLST = 5
BOOK_ROW_UUID = 6
BOOK_ROW_MISSING_SINCE_DATE = 7
# app.py _books_changed: グリッド用 book dict の path キー（ソート・比較で参照）
BOOK_DICT_KEY_PATH = "path"
# ui/widgets/menubar.py: 「見つからない本を表示...」OFF=通常グリッドでは非表示、ON=missing のみ表示
VIEW_MENU_MISSING_BOOKS_ACTION_LABEL = "見つからない本を表示..."
# app.py _apply_filters: メニューOFF時に missing パス（missing_since 付き）を一覧から除外するか（True=既定）
GRID_EXCLUDE_MISSING_PATHS_WHEN_MISSING_VIEW_OFF = True
...
# キュー一覧が空のときだけ表示するプレースホルダー行の文言（1 件でも受信すると消える）
BOOKMARKLET_QUEUE_EMPTY_PLACEHOLDER = (
    "【はじめに】Noble Shelf を起動したままにしてください（閉じると送信できません）。"
    "\nこのパソコン上のブラウザで作品ページを開き、下の「ブックマークレットをコピー」→ ブラウザにブックマーク登録 → ページ上でそのブックマークをクリック、の順です。"
    "\nくわしい手順は「？」のヘルプを開いてください。"
)
# プレースホルダー行の QListWidgetItem UserRole（実レコードの id と区別）
BOOKMARKLET_QUEUE_PLACEHOLDER_ROW_ID = -1
...
# 本文が増えたため最小高をやや確保（初心者向けヘルプ）
BOOKMARKLET_HELP_DIALOG_MIN_SIZE = (560, 420)
BOOKMARKLET_HELP_DIALOG_SIZE = (680, 600)
...
# drop_handler.py: 重複・行方不明（missing_since）再登録確認
DROP_DUPLICATE_DIALOG_TITLE = "重複"
DROP_DUPLICATE_FOLDER_EXISTS_TEMPLATE = "「{name}」は既に存在します。"
DROP_DUPLICATE_FOLDER_REGISTERED_TEMPLATE = "「{name}」は既に登録済みです。"
DROP_DUPLICATE_ARCHIVE_OR_REGISTERED_TEMPLATE = "「{name}」は既に存在または登録済みです。"
DROP_DUPLICATE_STORE_FILE_EXISTS_TEMPLATE = "「{name}」は既にライブラリに存在します。"
DROP_DUPLICATE_STORE_FILE_REGISTERED_TEMPLATE = "「{name}」は既に登録済みです。"
DROP_MISSING_BOOK_REDIALOG_TITLE = "行方不明の本"
DROP_MISSING_BOOK_REDIALOG_MESSAGE = "行方不明の本が見つかりました。再登録しますか？"
# drop_handler.py: Windows パス長（Win32 長パスプレフィックスなしの安全上限、文字数）
WINDOWS_MAX_PATH = 255
DROP_PATH_TOO_LONG_MESSAGE = (
    "パスが長すぎて登録できません。\n"
    "フォルダ名またはライブラリパスを短くしてください。\n\n"
    "対象: {path}"
)
...
LIBRARY_FOLDER_DIALOG_APP_DATA_CONFLICT_MESSAGE = (
    "このフォルダはNobleShelfのデータフォルダと競合するため選択できません"
)
LIBRARY_FOLDER_DIALOG_WINDOW_WIDTH_PX = 500
LIBRARY_FOLDER_DIALOG_WINDOW_HEIGHT_PX = 190
LIBRARY_FOLDER_DIALOG_SIZE = (
    LIBRARY_FOLDER_DIALOG_WINDOW_WIDTH_PX,
    LIBRARY_FOLDER_DIALOG_WINDOW_HEIGHT_PX,
)
LIBRARY_FOLDER_DIALOG_WARNING_ICON_DISPLAY_PX = 24
LIBRARY_FOLDER_DIALOG_WARNING_ROWS_SPACING_PX = 8
LIBRARY_FOLDER_DIALOG_WARNING_ROW_QSS = "background: transparent;"
LIBRARY_FOLDER_DIALOG_CHANGE_WARNING_LINE1_TEXT = (
    "フォルダ変更後のデータ整合性は保証されません"
)
LIBRARY_FOLDER_DIALOG_CHANGE_WARNING_LINE2_TEXT = (
    "問題が発生した場合はバックアップフォルダから復元してください"
)

# app.py MainWindow: ファイルメニュー「ライブラリの再スキャン」QAction の属性名（menubar 構築と一致）
MAIN_WINDOW_FILE_RESCAN_ACTION_ATTR = "_act_file_rescan"
...
# app.py _do_backup_and_quit: 終了時日次バックアップ失敗（logging.warning のフォーマット文字列）
LOG_BACKUP_QUIT_FAILURE_TEMPLATE = "[BACKUP] 終了時バックアップ失敗: %s"
# db.py init_db: マイグレーション前バックアップ失敗
LOG_DB_PRE_MIGRATION_BACKUP_FAILURE_TEMPLATE = (
    "[DB] マイグレーション前バックアップ失敗: %s"
)
# db.py _safe_backup: os.replace リトライ設定
SAFE_BACKUP_REPLACE_RETRY_COUNT = 3
SAFE_BACKUP_REPLACE_RETRY_WAIT_SEC = 0.5
# db.py _safe_backup: リトライ全失敗時の警告ログ
LOG_SAFE_BACKUP_REPLACE_FAILURE_TEMPLATE = (
    "[BACKUP] バックアップファイルの確定に失敗しました (tmp=%s)"
)
# db.py init_db: 起動バージョン記録失敗
LOG_DB_SET_LAUNCH_VERSION_FAILURE_TEMPLATE = (
    "[DB] 起動バージョンの記録に失敗しました: %s"
)
# drop_handler.py _get_pdf_cover_and_pages: PDF処理失敗
LOG_PDF_COVER_FAILURE_TEMPLATE = (
    "[PDF] カバー画像・ページ数の取得に失敗しました (path=%s): %s"
)
# store_file_resolver.build_db_index: 壊行スキップ（path, error）
LOG_RESOLVER_DB_ROW_SKIP_TEMPLATE = "[RESOLVER] DB行スキップ path=%s error=%s"
